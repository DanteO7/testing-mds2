...
describe("UserService - Unit Tests", () => {
  ...
  describe("updateUser", () => {
    ...
    it("debería rechazar edad inválida", async () => {
      const invalidUpdateData = { ...updateData, age: -5 };

      await expect(
        userService.updateUser(userId, invalidUpdateData)
      ).rejects.toThrow("Edad debe estar entre 0 y 120 años");

      expect(mockUserRepository.update).not.toHaveBeenCalled();
    });

    it("debería rechazar nombre inválido", async () => {
      const invalidUpdateData = { ...updateData, name: "d" };
    ...
    it("debería rechazar email duplicado", async () => {
      // este es el supuesto usuario que encuentra, que debe tener un id diferente pero el mismo mail para que tire el error
      const existingUser: User = {
        id: "2",
        name: "Juan Pérez",
        email: "juan@example.com",
